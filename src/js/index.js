...
// 5) menuWrap_start
//menu is imported from menu.js
let menuData = ``;
let iconData = ``;
let bannerData = ``;

for (let i = 0; i < banners.length; i++) {
  console.log(i, banners[i].imageDescription);
...
for (let i = 0; i < menu.length; i++) {
  menuData += `<div class="swiper-slide menuSlide menu${
    i + 1
  }" style="background-image: url(${menu[i].imageLink});">
...
for (let i = 0; i < icons.length; i++) {
  iconData += `<li>
  <a href="#"
  ><img src=${icons[i].imageLink} alt=${icons[i].imageDescription}
  /></a>
</li>`;
}

$(".menu-slides").html(menuData);
$(".icons").html(iconData);
...
const menuSwiper = new Swiper(".menu-swiper", {
  ...
  // Navigation arrows
  navigation: {
    nextEl: ".swiper-button-next.menuRight",
    prevEl: ".swiper-button-prev.menuLeft",
  },
});

const menuSlide = document.querySelector(".menu-swiper").swiper;
...
function createMap(idx) {
  ...
  const options = {
    center: new kakao.maps.LatLng(
      searchResultData[idx].y,
      searchResultData[idx].x
    ),
    level: 3,
  };
  ...
  if (searchResultData[idx].road_address_name) {
    var infowindow = new kakao.maps.InfoWindow({
      content:
        ...
        searchResultData[idx].place_name +
        "</div>",
    });
  }
